...
def create_log(tracker_id: int, timestamp: datetime, log: schemas.HabitLogCreate, db: Session = Depends(get_db)):
    tracker = db.query(models.Tracker).filter(models.Tracker.id == tracker_id).first()
    if not tracker:
        raise HTTPException(status_code=404, detail="Tracker not found")
    
    db_log = models.HabitLog(**log.model_dump(), tracker_id=tracker_id, timestamp=timestamp)
    db.add(db_log)
    db.commit()
    db.refresh(db_log)
    return db_log
